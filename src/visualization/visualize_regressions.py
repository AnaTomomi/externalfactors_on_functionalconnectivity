    ...
    for row, factor in enumerate(external_factors):
        ...
        for col in range(4):
            ...
            npb.plot(template='MNI152NLin2009cAsym',
                         template_style='glass', template_glass_maxalpha=0.2,
...
if len(ge)==0 and len(pc)>0 and len(links)>0: 
    ...
    grid_3d = plt.GridSpec(2, 4, wspace=-0.1, hspace=-0.1, left=0.05, right=0.8)
    grid_2d = plt.GridSpec(4, 1, wspace=0.1, hspace=0.1, left=0.81, right=0.95)
    axes_3d, axes_2d = [],[]
    ...
    for row in range(2):
        for col in range(4):
            ax = fig.add_subplot(grid_3d[row, col], projection='3d')
            ax.set_title(f'3D Plot {row*4 + col + 1}')
            axes_3d.append(ax)
    for row in range(4):
        if row == 0:
            ax2 = fig.add_subplot(grid_2d[row:2, 0], projection='3d')
            ax2.set_title(f'3D Plot in 2D Grid {row + 1}')
            axes_2d.append(ax2)
        elif row ==2:
            ax2 = fig.add_subplot(grid_2d[row, 0])
            ax2.set_title(f'2D Plot in 2D Grid {row + 1}')
            axes_2d.append(ax2)
        else:
            ax2 = fig.add_subplot(grid_2d[row, 0])
    ...
    for row,net in enumerate(networks):
        ...
        npb.plot(nodes=nodes_df, node_scale=50, title=None, fig=fig, ax=ax,
                 template='MNI152NLin2009cAsym', template_style='glass', template_glass_maxalpha=0.2)
