...
savepath = '/m/cs/scratch/networks-pm/pm_denoise'#sys.argv[2]
...
for file in filtered_files:
    ...
    #Button presses are stored as "Response" in the event_type column. Let's copy those to the code column
    pulse_info.loc[pulse_info['Event Type'] == 'Response', 'Code'] = pulse_info['Event Type']
    ...
    other_rows['duration'] = other_rows['duration']/10
    ...
    #Set the correct duration of the last row (the n-back feedback run longer than the MRI)
    dur = (nii_length*tr)-other_rows.iloc[-1].at["onset"]
    other_rows.iloc[-1, other_rows.columns.get_loc('duration')] = dur

    df = pd.concat([first_row, other_rows], ignore_index=True)
    
    #compute the duration of the Response events
    df['diff'] = df['onset'].shift(-1) - df['onset']
    df['duration'].fillna(df['diff'].shift(1), inplace=True)
    df.drop(columns=['diff'], inplace=True)
    
    replacement_dict = {'dual_block': 'block_start','stimulus1': 'oneback',
                        'stimulus2': 'twoback','Response': 'button_press'}
    df['trial_type'] = df['trial_type'].replace(replacement_dict)
    
    df.to_csv(f'{savepath}/{subject}/{subject}_task-nback_events.tsv', sep='\t', index=False)
