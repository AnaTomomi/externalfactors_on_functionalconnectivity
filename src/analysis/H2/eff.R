...
# Loop through the first num_net columns, i.e. each global efficiency per network
result_df <- data.frame()
for (i in 1:num_nets) {
  col_name <- colnames(data)[i] # Extract column name
  formula_str <- paste0(col_name, " ~ total_sleep_duration + awake_time + restless_sleep + steps + inactive_time") # Construct the formula dynamically
...
save_file = paste(save_path, paste0("global-eff_", strategy, "_", atlas_name, "_", thres, ".csv"), sep="/")
